...
function registerSocketHandlers(io) {
  ...
  io.on('connection', (socket) => {
    ...
    // Host types the birthday person's name + pronoun (lobby only)
    hostAction('set_celebrant', (game, payload) => M.setCelebrant(game, payload));

    // Host edits / adds questions for a round, or sends content: null to restore the defaults (lobby only)
    hostAction('set_questions', (game, { round, content }) => M.setRoundContent(game, round, content));
