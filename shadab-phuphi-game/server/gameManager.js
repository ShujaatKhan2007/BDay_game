 ...
 *        ├── celebrant: { name, shortName, pronoun } | null   // who the birthday challenge is for (typed by the Host)
 *        ├── customContent: { 1:{questions}, 3:{situations}, ... }  // Host-edited questions; a round that is
 *        │                                                        // missing here uses the default questions
...
const { validateName, validateCelebrant, validateRoundContent, SUBMISSION_VALIDATORS } = require('./validation');
...
/* ------------------------------------------------------------------ *
 *  Host controls: whose birthday, and rounds
 * ------------------------------------------------------------------ */

/** Host types the birthday person's name (and pronoun). Only possible in the lobby. */
function setCelebrant(game, payload) {
  if (game.stage !== 'lobby') return fail("You can only set the birthday person's name before the game starts.");
  const v = validateCelebrant(payload || {});
  if (!v.ok) return v;
  game.celebrant = v.value;
  touch(game);
  return { ok: true };
}

/** Host edits one round's questions (lobby only). content = null goes back to the defaults. */
function setRoundContent(game, round, content) {
  if (game.stage !== 'lobby') return fail('Questions can only be changed before the game starts.');
  if (!ROUNDS.includes(round)) return fail('Unknown round.');
  if (content === null) {
    delete game.customContent[round];
    touch(game);
    return { ok: true };
  }
  const v = validateRoundContent(round, content);
  if (!v.ok) return v;
  game.customContent[round] = v.value;
  touch(game);
  return { ok: true };
}
...
function startGame(game) {
  ...
  if (!game.celebrant) return fail("Enter the birthday person's name first.");
...
function submit(game, participant, round, payload) {
  ...
  // How many questions / situations does THIS game have? (Host may have added or removed some)
  const ctx = {
    qaCount: game.customContent[1]?.questions.length ?? CFG.QA_COUNT,
    reactionCount: game.customContent[3]?.situations.length ?? CFG.REACTION_COUNT,
  };
  const v = SUBMISSION_VALIDATORS[round](payload || {}, ctx);
  if (!v.ok) return v;
...
function hostView(game) {
  ...
  return {
    role: 'host',
    code: game.code,
    celebrant: game.celebrant ? { ...game.celebrant } : null,
    customContent: game.customContent,
    stage: game.stage,
