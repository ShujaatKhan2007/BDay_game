...
async function main() {
  ...
  /* ---------- Round 1 ---------- */
  await expectError(emit(players.Asha, 'submit_answer', { answers: ['only one'] }), 'all', 'incomplete answers');
  for (const name of names) {
    const r = await emit(players[name], 'submit_answer', { answers: ['60', 'Guddu', 'Chocolate'] });
  ...
  assert.deepStrictEqual(host.submissions[info.Asha.participantId][1].answers, ['60', 'Guddu', 'Chocolate']);
  ...
  const submitters = {
    ...
    3: ['submit_reaction', { reactions: [{ emoji: '😡', text: '' }, { emoji: '', text: 'Chup!' }, { emoji: '🛍️', text: 'Happy' }] }],
