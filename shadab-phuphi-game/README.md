...
One person is the **Host** (runs the game, chooses whose birthday it is, scores answers, reveals winners). Everyone else is a **Participant** who joins with a code or a WhatsApp link.

It works for **anyone's birthday**. In the lobby the Host types the birthday person's name (for example *Shadab Phuphi*, *Iqbal Fufa* or *Rahul*) and every question in all five rounds uses that name.
...
In the Host lobby, the **"Whose birthday is it? 🎂"** card asks for:

| Field | Example | Used for |
| --- | --- | --- |
| **Name** | `Shadab Phuphi` | Every question, the round titles and the celebration screen |
| **Short name** *(optional)* | `Phuphi` | The three Round 3 situations ("Phuphi when she can't find something."). Leave it empty to use the full name. |
| **She / He / They** | `She / her` | So questions say "her", "his" or "their" (for example "What was her nickname?") |

A live **Preview** shows how the questions will read. Press **SAVE**: every participant's lobby updates instantly ("Birthday challenge for Shadab Phuphi 🎂"). **START GAME** stays disabled until the name is saved, and it also locks the name (it can't be changed once the game starts).

For example, with *Iqbal Fufa* / *Fufa* / *He*: "How old is Iqbal Fufa turning this birthday?", "What was his nickname?", "Fufa when he can't find something.", "Write an Iqbal Fufa dialog which you know."

To play a second person's birthday afterwards: press **🔄 Play again** (same code and players, answers and scores cleared), type the new name in the lobby and save. The previous leaderboard is gone after this, so note the winners down first.

### Editing or adding questions

Below the name form, a collapsible **"✏️ Edit questions"** card lets the Host change any round before the game starts:

* Open a round to see its questions ready to edit. Change the text, or use **+ ADD QUESTION** / **+ ADD SITUATION** (1–10 questions in Round 1, 1–6 situations in Round 3). A ✕ button removes one (at least one must remain).
* For Round 3, tap which emoji choices to offer for each situation; participants can also always type a custom reaction.
* Press **SAVE ROUND N QUESTIONS**. Every participant's screen updates instantly, and the round's tab in the card shows an "edited" label.
* **↩ USE DEFAULT QUESTIONS** removes your changes for that round and goes back to the built-in text.
* Editing is only possible in the lobby; once START GAME is pressed the questions are locked for that game.
...
games (Map)
 └── "PUPPY25"
      ...
      ├── celebrant          { name, shortName, pronoun } or null  (typed by the Host in the lobby)
      ├── customContent      { 1:{questions}, 3:{situations}, 2/4/5:{prompt} }  (Host-edited questions;
      │                        a round missing here uses the default questions)
...
| **Birthday name** | Host lobby: type a name, pick She/He/They, press SAVE | Preview updates as you type; participants' lobby shows "Birthday challenge for …"; START GAME is disabled until saved |
| **Custom name in rounds** | Save "Iqbal Fufa" / "Fufa" / He, start, open each round | Questions say "Iqbal Fufa", "his nickname", "FUFA'S REACTION CHALLENGE" |
| **Edit / add questions** | Open "Edit questions" → edit a question, add another, save | Preview and participant screens show your text and the new question |
...
* **Questions, prompts, emojis:** `client/src/config/gameContent.js`
  The questions are templates that insert the Host's name, short name and pronoun, for example `` `What was ${possessive} nickname?` ``. Edit the wording there and it applies to every birthday person.
  Keep the *number* of items (3 questions in Round 1, 3 situations in Round 3), because the server expects exactly that many answers. **Party guests don't need this file at all** - the Host can edit or add questions for any single game straight from the lobby (see "Editing or adding questions" above); this file only changes the built-in defaults everyone starts from.
* **Question limits:** `server/gameConfig.js` (`MAX_QUESTIONS`, `MAX_SITUATIONS`, `MAX_EMOJIS`) and matching text lengths in `LIMITS`.
...
| `set_celebrant` | Host only | `{name, shortName, pronoun}` (pronoun is `she`, `he` or `they`), lobby only |
| `set_questions` | Host only | `{round, content}` - edits/adds questions for a round; `content: null` restores the default, lobby only |
| `start_game` | Host only | Lobby → Round 1 |
| `change_round` | Host only | `{direction: 'next'}` or `{direction: 'prev'}` |
