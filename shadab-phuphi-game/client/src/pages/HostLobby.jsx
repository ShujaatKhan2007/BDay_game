...
import { APP } from '../config/gameContent.js';
import { copyText } from '../utils/clipboard.js';
import Puppy from '../components/Puppy.jsx';
import CelebrantForm from '../components/CelebrantForm.jsx';
...
export default function HostLobby({ state }) {
  ...
  const [formSaved, setFormSaved] = useState(false); // is what's typed in the name form saved?
  const onFormStatus = useCallback((saved) => setFormSaved(saved), []);

  // The link works on whatever address this app is hosted at.
  const joinUrl = `${window.location.origin}/join/${state.code}`;
  const whatsapp = `https://wa.me/?text=${encodeURIComponent(
    `🎂 Join the Birthday Challenge${event.celebrant ? ` for ${event.celebrant}` : ''}!\nGame code: ${state.code}\n${joinUrl}`,
  )}`;
    ...
    <main className="screen">
      <div className="wrap">
        <section className="marquee hero compact">
          ...
          <h1 className="hero-title small">🎂 {APP.titleTop} {APP.titleBottom}</h1>
        </section>

        <CelebrantForm celebrant={state.celebrant} onStatus={onFormStatus} />

        <QuestionsSection celebrant={state.celebrant} customContent={state.customContent} />
        ...
        <div className="lobby-grid">
          ...
          <section className="card people-card">
            ...
            <button
              className="btn pink xl block"
              onClick={start}
              disabled={busy || state.participants.length === 0 || !formSaved}
            >
              START GAME 🚀
            </button>
            <p className="muted small center-text">
              {formSaved
                ? <>Starting the challenge for <strong>{event.celebrant}</strong></>
                : "Enter and SAVE the birthday person's name first."}
            </p>
          </section>
        </div>
